...
public class PlayerManagement extends AbstractManagement {
    ...
    public void add(String club, String name) {
		...
		if (root() != 0) {
			...
			if (fit != 0) {
				...
				String meta = VISIBLE_INDICATOR + String.format("%02d", sizeOfFit) + club + String.format("%04d", founds[1]);
		...
		String meta = VISIBLE_INDICATOR + String.format("%02d", name.length()) + club + String.format("%04d", founds[1]);
		data.append(meta);
		data.append(name);
	...
	public void deletes(int club) {
		...
		while (position != 0) {
			...
			data.setCharAt(position, DELETED_INDICATOR);
			data.replace(position + 6, position + 10, emptyPattern());
			data.replace(0, 4, String.format("%04d", position));
